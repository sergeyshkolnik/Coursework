...
function build_html_calendar($year, $month, $events = null) {
    ...
    for ($day = 1; $day <= $days_in_month; $day++) {
      ...
      if ($draw_event) {
        ...
        "<form action = 'viewcoachmeetings.php' method = 'POST'>
  ...
  $stmt = $conn->prepare("SELECT MeetingID, StartTime, Forename, Surname FROM TblMeetings
  INNER JOIN TblUsers ON TblMeetings.UserID = TblUsers.UserID
  WHERE Agreed_by_coach = 1");
  ...
  while ($row = $stmt->fetch(PDO::FETCH_ASSOC)){
    ...
    {
        $events[$date]['text']=$events[$row[$date]]['text']."<br>".$time." <p> with</p> ".$row['Surname']." ".$row['Forename'];
        
    }else{
        $events[$date]=array('text'=>$time." <p> with</p> ".$row['Surname']." ".$row['Forename']);
